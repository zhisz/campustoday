...
def create_client(session_cookie=None, device_profile=None) -> AttendanceClient:
    mode = os.getenv("CPDAILY_MODE", "disabled").strip().lower()
    if mode == "jxust":
        return JxustAttendanceClient(session_cookie=session_cookie, device_profile=device_profile)
    if mode == "disabled":
